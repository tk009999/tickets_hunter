...
2. 執行 TicketsHunter.exe
   雙擊 TicketsHunter.exe，會直接開啟中文 Windows 桌面程式。
...
4. 點擊「開始搶票」
   系統會在本機開啟 Chrome，狀態與日誌會顯示在桌面程式內。

================================================================================
                        系統架構說明
================================================================================

【TicketsHunter.exe - Windows 桌面主程式】
  使用 .NET 8 WPF 原生 Windows 控制項，不會開啟網頁設定介面。
  ...
  執行方式：
    雙擊 TicketsHunter.exe → 直接開啟 Windows 桌面視窗
...
【搶票引擎（自動啟動，無需手動執行）】
  ...
  此執行檔放在 `_engine` 目錄，會由 TicketsHunter.exe 自動啟動，
  一般使用者無需手動執行。
